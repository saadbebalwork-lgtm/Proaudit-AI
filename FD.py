...
def create_pdf_report(
    ...
    story.append(Paragraph("Generated By: " + username, styles["Normal"]))
    story.append(Paragraph("Client: " + client_name, styles["Normal"]))
    story.append(Paragraph("Generated At: " + datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC"), styles["Normal"]))
    story.append(Spacer(1, 10))
    story.append(Paragraph("Metric: " + selected_metric, styles["Normal"]))
    story.append(Paragraph("Anomalous Value: " + str(anomaly_value), styles["Normal"]))
    story.append(Paragraph("Z-Score: " + str(round(float(anomaly_zscore), 2)), styles["Normal"]))
    ...
    with left:
        st.markdown(
            ...
            <div class="hero-card">
                ...
                <div class="feature-bullet">• Multi-client workspace management</div>
...
if all_clients:
    ...
    if st.session_state.selected_client_name in client_name_map:
        default_client_name = st.session_state.selected_client_name
    else:
        default_client_name = all_clients[0]["client_name"]
    ...
    if st.button("Add client", use_container_width=True):
        if not new_client_name.strip():
            ...
            if created:
                ...
                st.error("Could not create client. Check Supabase table/policies.")
...
with header_right:
    st.markdown(
        f"""
        <div class="mini-card" style="text-align:center; margin-top:0.2rem;">
...
# Require client before continuing
    ...
    with c2:
        ...
        if recent_runs:
            for run in recent_runs[:5]:
                st.markdown(
                    f"""
                    <div class="small-muted" style="margin-bottom:0.6rem;">
                    • {run.get('file_name', 'Unknown file')} — {run.get('risk_label', 'N/A')} risk — {run.get('anomaly_count', 0)} anomalies
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
        else:
            st.info("No saved runs yet for this client.")
        st.markdown("</div>", unsafe_allow_html=True)
...
st.markdown(
    ...
    <div class="status-card">
        ...
        <span class="small-muted"> — File loaded successfully for client: {st.session_state.selected_client_name}</span>
    </div>
    """,
    unsafe_allow_html=True,
)
...
if page == "📊 Dashboard":
    ...
    with i1:
        ...
        st.markdown(
            ...
            • Current workspace: <b>{st.session_state.selected_client_name}</b>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with i2:
        ...
        if recent_runs:
            for run in recent_runs[:5]:
                ...
                st.markdown(
                    f"""
                    <div class="small-muted" style="margin-bottom:0.55rem;">
                    • {run.get('file_name', 'Unknown file')} — {run.get('risk_label', 'N/A')} risk — {run.get('anomaly_count', 0)} anomalies
                    </div>
                    """,
                    unsafe_allow_html=True,
